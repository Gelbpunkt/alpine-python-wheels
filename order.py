...
def clean_requirements(orderlist, keylist, requirements):
    requirements = [req for req in requirements if req in keylist and not req in orderlist]
    return requirements


if __name__ == '__main__':
    whl_folder = 'wheels'
    whl_files = [file for file in listdir(whl_folder) if isfile(join(whl_folder, file)) and file.endswith('.whl')]
    ...
    for file in whl_files:
        ...
        version = StrictVersion(metadata.version)
        if (current_version := name_to_version.get(metadata.name, None)) is not None and version > current_version:
            whl_metadatas.append(metadata)
            name_to_file[metadata.name] = file
            name_to_version[metadata.name] = version

    requirements = dict(
        (clean_name(md.name), [clean_name(req.split()[0]) for req in md.requires_dist]) for md in whl_metadatas)
